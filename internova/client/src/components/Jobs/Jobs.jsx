...
const Jobs = () => {
  const { allJobs = [], searchedQuery } = useSelector((store) => store.job); // Add default empty array
  const [filterJobs, setFilterJobs] = useState([]); // Initialize with empty array
  const jobsPerPage = 10; // Limit number of jobs per page
  ...
  useEffect(() => {
    if (searchedQuery) {
      ...
      setFilterJobs(filteredJobs.slice(0, jobsPerPage)); // Limit to first 10 jobs
    } else {
      setFilterJobs(allJobs.slice(0, jobsPerPage)); // Limit to first 10 jobs when no search
    }
  }, [allJobs, searchedQuery]);
